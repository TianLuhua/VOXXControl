...
import com.sat.mirrorcontorl.utils.ServiceUtils;
import com.sat.mirrorcontorl.widget.VerticalSeekBar;
import com.sat.mirrorcontorl.widget.VerticalSeekBar.OnSeekBarChangeListener;
...
public class WifiActivity extends Activity {
	...
	private Handler wifiHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GET_SCAN_RESULT:
				...
				if (scanResults != null) {
					...
					if (mListAdapter != null) {
						...
						int max = availableResults.size() > 6 ? availableResults
								.size() - 6 : availableResults.size();
			...
			case SHOE_HAS_CONNECT_DIALOG:
				...
				if (isConnect == 2) {
					buider.setNeutralButton("Forget",
							new AlertDialog.OnClickListener() {

								@Override
								public void onClick(DialogInterface dialog,
										int which) {
									wifiManager.removeNetwork(wifiManager
											.getConfiguredNetworks().get(
													position).networkId);
								}
							});
				}
				buider.setNegativeButton(positionText,
						new AlertDialog.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								if (positionText.equals("Connect")) {
									wifiAdmin.addNetwork(wifiManager
											.getConfiguredNetworks().get(
													position));
									tvWifiState
											.setText(R.string.wifi_state_connect);
								} else {
									wifiAdmin.disconnectWifi(wifiManager
											.getConfiguredNetworks().get(
													position).networkId);
								}
								hasLinkShow.dismiss();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 如何MediaCodeService在Launcher中没有启动，这里就再次启动
		if (!ServiceUtils.isServiceRunning(getApplication(),
				MediaCodeService.class.getName())) {
			startService(new Intent("intent.action.mediacodeservice"));
		}
	...
	protected void onResume() {
		...
		if (networkInfo != null && networkInfo.isConnected()) {
			...
			tvDeviceinfo.setText("1.Use phone connect Wifi:"
					+ networkInfo.getExtraInfo()
					+ " \n2.Use phone connect wireless device: "
	...
	private void initLisener() {
		...
		tvWifiScan.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if (availableResults != null && availableResults.size() > 0
						&& mListAdapter != null) {
					availableResults.clear();
					mListAdapter.notifyDataSetChanged();
				}
		...
		tvWifiStart.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				startActivityformComponent("cn.manstep.phonemirror",
						"cn.manstep.phonemirror.MainActivity");
				finish();
			}
		});
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (configuredNetworks != null) {
					for (int i = 0; i < configuredNetworks.size(); i++) {
						...
						String newSsid = "\"" + ssid + "\"";
						System.out.println("ssid:" + newSsid + "  ssid2:"
								+ ssid2);
						System.out.println("ssid:" + newSsid + "  ssid3:"
								+ ssid3);
				...
				View infoView = View.inflate(getApplicationContext(),
						R.layout.dialog_wifi_pwd, null);
				CheckBox cbShowPwd = (CheckBox) infoView
						.findViewById(R.id.cb_show_pwd);
				etPwd_dialog = (EditText) infoView.findViewById(R.id.et_pwd);
				TextView tvPwdType = (TextView) infoView
						.findViewById(R.id.tv_pwd_type);
				...
				builder.setPositiveButton(R.string.wifi_dialog_cancel,
				...
				builder.setNegativeButton(R.string.wifi_dialog_connect,
						new DialogInterface.OnClickListener() {

							@Override
							public void onClick(DialogInterface dialog,
									int which) {
								String etPwd = etPwd_dialog.getText()
										.toString().trim();
								...
								if (!TextUtils.isEmpty(etPwd)) {
									if (capabilities.contains("WEP")) {
										wifiAdmin.addNetwork(wifiAdmin
												.CreateWifiInfo(ssid, etPwd, 2));
										tvWifiState
												.setText(R.string.wifi_state_connect);
									} else if (capabilities.contains("WPA")) {
										wifiAdmin.addNetwork(wifiAdmin
												.CreateWifiInfo(ssid, etPwd, 3));
										tvWifiState
												.setText(R.string.wifi_state_connect);
									} else {
										Toast.makeText(
												getApplicationContext(),
												R.string.wifi_dialog_toast_cant,
												Toast.LENGTH_SHORT).show();
									}
								} else {
									Toast.makeText(getApplicationContext(),
											R.string.wifi_dialog_toast,
											Toast.LENGTH_SHORT).show();
								}
								// }
	...
	private String getPwdType(int position) {
		...
		if (!TextUtils.isEmpty(capabilities)) {
			if (capabilities.contains("WPA-PSK")
					&& capabilities.contains("WPA2-PSK")) {
				cureentPwdType = "WAP-PSK/WAP2-PSK";
			} else if (capabilities.contains("WPA-")
					&& capabilities.contains("WPA2-")) { // 杈冧笉瀹夊叏
				cureentPwdType = "WAP/WAP2";
			} else if (capabilities.contains("WEP")) { // 杈冧笉瀹夊叏
				cureentPwdType = "WEP";
	...
	private class MyListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				viewHolder.tvWifiName = (TextView) convertView
						.findViewById(R.id.tv_wifi_name);
				viewHolder.tvWifiLevel = (TextView) convertView
						.findViewById(R.id.tv_wifi_level);
				viewHolder.ivWifiLevel = (ImageView) convertView
						.findViewById(R.id.iv_wifi_level);
				viewHolder.tvWifiPwd = (TextView) convertView
						.findViewById(R.id.tv_wifi_pwd);
			...
			if (!TextUtils.isEmpty(capabilities)) {
				if (capabilities.contains("WPA-PSK")
					...
					viewHolder.tvWifiPwd
							.setText(R.string.wifi_secured_wpa2andwpa1);
			...
			if (hasPwd) {
				if (-availableResults.get(position).level >= 80) {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_lock_signal_1_dark);
				} else if (-availableResults.get(position).level >= 70) {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_lock_signal_2_dark);
				} else if (-availableResults.get(position).level >= 55) {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_lock_signal_3_dark);
				} else {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_lock_signal_4_dark);
				}
			} else {
				if (-availableResults.get(position).level >= 80) {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_signal_1_dark);
				} else if (-availableResults.get(position).level >= 70) {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_signal_2_dark);
				} else if (-availableResults.get(position).level >= 55) {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_signal_3_dark);
				} else {
					viewHolder.ivWifiLevel
							.setImageResource(R.drawable.ic_wifi_signal_4_dark);
				}
			}
	...
	private class MyReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (scanResults != null) {
				...
				sortByLevel(availableResults);
				lvWifi.setAdapter(mListAdapter);
				int max = availableResults.size() > 6 ? availableResults.size() - 6
		...
		public void onReceive(Context arg0, Intent intent) {
			...
			if (networkInfo != null && networkInfo.isConnected()) {
				...
				if (extraInfo != null && extraInfo.length() > 0) {
					extraInfo = (String) extraInfo.subSequence(1,
							extraInfo.length() - 1);
					System.out.println("extraInfo:" + extraInfo);
				}
