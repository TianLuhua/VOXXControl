package com.sat.mirrorcontorl.screencaputre;
...
public class ScreenCaputre {
	...
	public void setScreenCaputreListener(
			ScreenCaputreListener screenCaputreListener) {
	...
	public void prepareVideoEncoder() throws IOException {
		...
		MediaCodec vencoder = MediaCodec
	...
	public void startVideoEncode() {
		if (vEncoder == null) {
			throw new RuntimeException("请初始化视频编码�?");
		}
		if (videoEncoderLoop) {
			throw new RuntimeException("必须先停�?");
		}
		videoEncoderThread = new Thread() {
			...
			public void run() {
				...
				while (videoEncoderLoop && !Thread.interrupted()) {
					try {
						ByteBuffer[] outputBuffers = vEncoder
								.getOutputBuffers();
						int outputBufferId = vEncoder.dequeueOutputBuffer(
								vBufferInfo, 0);
						if (outputBufferId >= 0) {
	...
	private void onEncodedAvcFrame(ByteBuffer bb,
			final MediaCodec.BufferInfo vBufferInfo) {
